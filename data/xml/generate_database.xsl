...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="/">
        <!-- ======================= THE FIX IS HERE ======================= -->
        <!-- Restoring the full list of coin variables from your example -->
        ...
        <xsl:variable name="totalPersons" select="count(//tei:persName[@xml:id or starts-with(@ref, '#auth_pers_')][generate-id(.) = generate-id(key('persons-by-unified-id', if (starts-with(@ref, '#auth_pers_')) then substring-after(@ref, '#auth_pers_') else replace(replace(@xml:id, '^pers_', ''), '_jer.*$', ''))[1])])"/>
        ...
        <xsl:for-each select="//tei:persName[@xml:id or starts-with(@ref, '#auth_pers_')][generate-id(.) = generate-id(key('persons-by-unified-id', if (starts-with(@ref, '#auth_pers_')) then substring-after(@ref, '#auth_pers_') else replace(replace(@xml:id, '^pers_', ''), '_jer.*$', ''))[1])]">
            <xsl:variable name="unified_id" select="if (starts-with(@ref, '#auth_pers_')) then substring-after(@ref, '#auth_pers_') else replace(replace(@xml:id, '^pers_', ''), '_jer.*$', '')"/>
            <xsl:if test="$unified_id != ''">
                <xsl:variable name="current-group" select="key('persons-by-unified-id', $unified_id)"/>
                <xsl:variable name="primary_node" select="$current-group[1]"/>
                <xsl:variable name="primaryRole">
                    <xsl:choose>
                        <xsl:when test="$primary_node/@role='donor'">donor</xsl:when>
                        <xsl:when test="$primary_node/@role='scribe'">scribe</xsl:when>
                        <xsl:otherwise>commemorated</xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>

                <xsl:text>{"id": "</xsl:text><xsl:value-of select="$unified_id"/><xsl:text>",</xsl:text>
                <xsl:text>"nameKa": "</xsl:text><xsl:call-template name="escape-json-string"><xsl:with-param name="text" select="$primary_node/@key"/></xsl:call-template><xsl:text>",</xsl:text>
                <xsl:text>"nameEn": "</xsl:text><xsl:call-template name="escape-json-string"><xsl:with-param name="text" select="$primary_node/@key"/></xsl:call-template><xsl:text>",</xsl:text>
                <xsl:text>"titleKa": "</xsl:text><xsl:call-template name="escape-json-string"><xsl:with-param name="text" select="normalize-space($primary_node)"/></xsl:call-template><xsl:text>",</xsl:text>
                <xsl:text>"titleEn": "</xsl:text><xsl:call-template name="escape-json-string"><xsl:with-param name="text" select="normalize-space($primary_node)"/></xsl:call-template><xsl:text>",</xsl:text>
                <xsl:text>"familyKey": "</xsl:text><xsl:call-template name="escape-json-string"><xsl:with-param name="text" select="fn:string-join($primary_node/tei:addName[@type='patronymic']/@key, ', ')"/></xsl:call-template><xsl:text>",</xsl:text>
                <xsl:text>"roleKey": "</xsl:text><xsl:call-template name="escape-json-string"><xsl:with-param name="text" select="fn:string-join($primary_node/tei:roleName/@key, ', ')"/></xsl:call-template><xsl:text>",</xsl:text>
                <xsl:text>"roleCategory": "</xsl:text><xsl:if test="$primary_node/tei:roleName/@type"><xsl:value-of select="substring-before(concat($primary_node/tei:roleName[1]/@type, '_'), '_')"/></xsl:if><xsl:text>",</xsl:text>
                <xsl:text>"primaryRole": "</xsl:text><xsl:value-of select="$primaryRole"/><xsl:text>",</xsl:text>
                <xsl:text>"sourcesDisplay": "</xsl:text><xsl:for-each select="$current-group"><xsl:variable name="msIdent" select="ancestor::tei:TEI//tei:msDesc/tei:msIdentifier"/><xsl:text>[</xsl:text><xsl:value-of select="substring(normalize-space($msIdent/tei:settlement), 1, 3)"/><xsl:text>. </xsl:text><xsl:value-of select="normalize-space($msIdent/tei:idno)"/><xsl:text>, </xsl:text><xsl:value-of select="ancestor::tei:item/@n"/><xsl:text>]</xsl:text><xsl:if test="position() != last()"><xsl:text>, </xsl:text></xsl:if></xsl:for-each><xsl:text>",</xsl:text>
                <xsl:text>"items": [</xsl:text><xsl:for-each select="$current-group"><xsl:text>"</xsl:text><xsl:value-of select="ancestor::tei:item/@xml:id"/><xsl:text>"</xsl:text><xsl:if test="position() != last()"><xsl:text>,</xsl:text></xsl:if></xsl:for-each><xsl:text>]}</xsl:text>
        ...
        <!-- ======================= THE FIX IS HERE ======================= -->
        <!-- Restoring the full list of coin template calls from your example -->
        <xsl:call-template name="generate-coin-data"><xsl:with-param name="isFirst" select="true()"/><xsl:with-param name="total" select="$totalFlorins"/><xsl:with-param name="key" select="'florin'"/><xsl:with-param name="labelKa" select="'ფლური'"/><xsl:with-param name="labelEn" select="'Florin'"/></xsl:call-template>
        <xsl:call-template name="generate-coin-data"><xsl:with-param name="isFirst" select="not($totalFlorins > 0)"/><xsl:with-param name="total" select="$totalTetri"/><xsl:with-param name="key" select="'tetri'"/><xsl:with-param name="labelKa" select="'თეთრი'"/><xsl:with-param name="labelEn" select="'Tetri'"/></xsl:call-template>
        <xsl:call-template name="generate-coin-data"><xsl:with-param name="isFirst" select="not($totalFlorins > 0 or $totalTetri > 0)"/><xsl:with-param name="total" select="$totalDrama"/><xsl:with-param name="key" select="'drama'"/><xsl:with-param name="labelKa" select="'დრაჰმა'"/><xsl:with-param name="labelEn" select="'Drama'"/></xsl:call-template>
        <xsl:call-template name="generate-coin-data"><xsl:with-param name="isFirst" select="not($totalFlorins > 0 or $totalTetri > 0 or $totalDrama > 0)"/><xsl:with-param name="total" select="$totalMarchili"/><xsl:with-param name="key" select="'marchili'"/><xsl:with-param name="labelKa" select="'მარჩილი'"/><xsl:with-param name="labelEn" select="'Marchili'"/></xsl:call-template>
        <xsl:call-template name="generate-coin-data"><xsl:with-param name="isFirst" select="not($totalFlorins > 0 or $totalTetri > 0 or $totalDrama > 0 or $totalMarchili > 0)"/><xsl:with-param name="total" select="$totalDrahkani"/><xsl:with-param name="key" select="'drahkani'"/><xsl:with-param name="labelKa" select="'დრაჰკანი'"/><xsl:with-param name="labelEn" select="'Drahkani'"/></xsl:call-template>
        <xsl:call-template name="generate-coin-data"><xsl:with-param name="isFirst" select="not($totalFlorins > 0 or $totalTetri > 0 or $totalDrama > 0 or $totalMarchili > 0 or $totalDrahkani > 0)"/><xsl:with-param name="total" select="$totalVenetianTetri"/><xsl:with-param name="key" select="'venetian_tetri'"/><xsl:with-param name="labelKa" select="'ვენეტიკური თეთრი'"/><xsl:with-param name="labelEn" select="'Venetian Tetri'"/></xsl:call-template>
        <xsl:call-template name="generate-coin-data"><xsl:with-param name="isFirst" select="not($totalFlorins > 0 or $totalTetri > 0 or $totalDrama > 0 or $totalMarchili > 0 or $totalDrahkani > 0 or $totalVenetianTetri > 0)"/><xsl:with-param name="total" select="$totalVelentiuriFlorin"/><xsl:with-param name="key" select="'velentiuri_florin'"/><xsl:with-param name="labelKa" select="'ველენტიური ფლური'"/><xsl:with-param name="labelEn" select="'Velentiuri Florin'"/></xsl:call-template>
        <xsl:call-template name="generate-coin-data"><xsl:with-param name="isFirst" select="not($totalFlorins > 0 or $totalTetri > 0 or $totalDrama > 0 or $totalMarchili > 0 or $totalDrahkani > 0 or $totalVenetianTetri > 0 or $totalVelentiuriFlorin > 0)"/><xsl:with-param name="total" select="$totalMutqaliGold"/><xsl:with-param name="key" select="'mutqali_gold'"/><xsl:with-param name="labelKa" select="'მუტყალი ოქრო'"/><xsl:with-param name="labelEn" select="'Mutqali Gold'"/></xsl:call-template>
        <xsl:call-template name="generate-coin-data"><xsl:with-param name="isFirst" select="not($totalFlorins > 0 or $totalTetri > 0 or $totalDrama > 0 or $totalMarchili > 0 or $totalDrahkani > 0 or $totalVenetianTetri > 0 or $totalVelentiuriFlorin > 0 or $totalMutqaliGold > 0)"/><xsl:with-param name="total" select="$totalDucats"/><xsl:with-param name="key" select="'ducat'"/><xsl:with-param name="labelKa" select="'დუკატი'"/><xsl:with-param name="labelEn" select="'Ducat'"/></xsl:call-template>
        <!-- =============================================================== -->
        <xsl:text>]</xsl:text>
        <xsl:text>}</xsl:text>
        <xsl:text>}</xsl:text>
    ...
    <xsl:template name="generate-coin-data">
        ...
        <xsl:param name="labelKa" as="xs:string"/>
        <xsl:param name="labelEn" as="xs:string"/>
        <xsl:if test="$total > 0">
